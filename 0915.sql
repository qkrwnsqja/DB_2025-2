...
--주문 테이블에서 주문고객별 수량의 평균을 주문고객을 기준으로 내림차순으로 검색하시오. (단 수량의 평균은 소수점 이하 첫째자리까지 출력)
select 주문고객, round(avg(수량), 1) "수량의 평균" from 주문 
                            group by 주문고객 
                            order by 주문고객 desc;
                            
select * from 고객;
--고객 테이블에서 등급별 적립금의 평균을 등급을 기준으로 오름차순으로 검색하시오.(단, 적립금의 평균은 소수점 이하 둘쨰자리까지 출력)
select 등급, round(avg(적립금), 2) "적립금의 평균" from 고객 
                                                 group by 등급
                                                 order by 등급;
...
-- 제품 테이블에서 [제품을 2개 이하 제조한] 제조업체별로 제조한 제품의 개수와 제품 중 가장 비싼 단가 중 [4000원 이상인 것]을 검색하시오.(단, 제품의 개수는 제품수, 가장 비싼 단가는 최고가라는 컬럼명으로 출력)
select 제조업체, count(*) 제품수, max(단가) 최고가 from 제품 
                                                    group by 제조업체
                                                    having count(*)<=2 and max(단가)>=4000;
                                                
-- 고객 테이블에서 [적립금 평균이 1000원 이상인 등급에 대해] 등급별 고객수와 적립금 평균을 검색하시오.
-- 단, 고객수는 고객수로 적립금 평균은 평균 적립급으로 출력, 평균적립금은 소수점 이하 둘째자리까지
...
select 주문제품, 제품명, 주문일자 from 주문, 제품, 고객
